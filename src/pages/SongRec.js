...
function SongRec({ token }) {
  const { trackId } = useParams(); // song ID
  const location = useLocation();
  const [trackName, setTrackName] = useState(location.state?.trackName || ""); // Track name from location state
  ...
  useEffect(() => {
    const fetchTrackName = async () => {
      try {
        ...
        setTrackName(trackResponse.data.name);
      } catch (err) {
    ...
    fetchTrackName(); // Fetch the track name
    fetchRecommendations(); // Fetch recommendations based on the track
  }, [trackId, token]);
    ...
    try {
      ...
      const newPlaylistResponse = await axios.post(
        `https://api.spotify.com/v1/users/${userId}/playlists`,
        {
          ...
          description: `A playlist of recommendations based on the song "${trackName}".`,
          public: isPublic,
        },
        {
          headers: {
    ...
    <div className="pl-48 pr-48 ml-24 pt-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white">
          Recommended Tracks based on <span className="text-[#1ED760]">{trackName}</span>
        </h2>
        <button
          onClick={openModal}
          disabled={saving}
          className={`px-4 py-2 bg-[#1ED760] text-xs mr-24 text-black rounded-full font-bold hover:bg-[#1db954] transition duration-300 ${
